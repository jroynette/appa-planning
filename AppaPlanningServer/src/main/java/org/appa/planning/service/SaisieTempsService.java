...
public class SaisieTempsService {
	 ...
	 * @param planning le planning hebdo à sauvegarder
	 */
		...
		for (SaisieTempsProjet saisieProjet : planning.getProjets()) {
			for (SaisieTempsProjetJour saisieProjetJour : saisieProjet.getTemps().values()) {
				if(saisieProjetJour.getHeures() != null && saisieProjetJour.getHeures() > 0){
					...
					saisieTemps.setProjet(saisieProjet.getProjet());
	...
	public SaisieTempsSemaine loadSaisieTemps(String login, Date date){
		...
		Map<String,SaisieTempsProjet> projetsSaisis = new HashMap<String,SaisieTempsProjet>();
		...
		//plage de dates entre début et fin de semaine de saisie (samedi)
		...
		//récupération des temps saisis du lundi au samedi sur la semaine
		List<SaisieTemps> saisies = saisieTempsRepository.findByUtilisateur(user.getId(), dateDebutSemaine, dateFinSemaine);

		//recuperation de la liste des projets de l'annee selectionnée
		//TODO voir le pb des semaines sur 2 années
