alter table Absence drop constraint FK1C341A1D40393A86
alter table DejeunerExterne drop constraint FKA0DF95C340393A86
alter table SaisieTemps drop constraint FKB3C98C4B184DF70E
alter table SaisieTemps drop constraint FKB3C98C4B40393A86
...
create table Absence (id integer generated by default as identity (start with 1), commentaire varchar(255), dateDebut timestamp not null, dateFin timestamp not null, debutAM bit not null, finPM bit not null, statut integer not null, type integer not null, utilisateur_id integer, primary key (id))
create table DejeunerExterne (id integer generated by default as identity (start with 1), date timestamp not null, utilisateur_id integer not null, primary key (id))
...
create table Utilisateur (id integer generated by default as identity (start with 1), actif bit not null, login varchar(20) not null, nbConges integer not null, nbRTT integer not null, nom varchar(30) not null, password varchar(10), prenom varchar(30) not null, quatrevingt bit not null, role varchar(15) not null, primary key (id), unique (login))
