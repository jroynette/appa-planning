...
public interface ReportingTempsRepository extends JpaRepository<SaisieTemps, Long> {
	...
	/**
	 * @return la repartition des saisies de temps par projet pour un utilisateur entre les 2 dates (triee de mani�re decroissante sur le nombre d'heures)
	 */
	@Query("select new org.appa.planning.vo.LigneReportingProjet(s.projet.nom, s.utilisateur.prenom || ' ' || s.utilisateur.nom, sum(s.heures)) " +
	...
	List<LigneReportingProjet> reportForUserByDates(Long utilisateurId, Date dateDebut, Date dateFin );
